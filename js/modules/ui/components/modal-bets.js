(function () {
        ...
        <div class="relative w-full max-w-2xl apple-glass rounded-[28px] shadow-[0_24px_80px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col max-h-[82vh] animate-apple-in border-white/5">
            ...
            <div id="userAnalyticsContent" class="flex-1 overflow-y-auto p-5 md:p-6 custom-scroll relative">
                ...
                <div class="grid grid-cols-3 gap-3 md:gap-4 mb-5 md:mb-6 relative z-10">
                    <div class="bg-white/[0.02] border border-white/[0.05] rounded-[16px] md:rounded-[20px] p-4 text-center shadow-xl group transition-all hover:bg-white/[0.04]">
                        <div class="text-[8px] font-black text-white/20 uppercase tracking-[0.2em] mb-1">Total Net</div>
                        <div id="userNet" class="text-2xl md:text-3xl font-black tracking-tighter text-white">0</div>
                    </div>
                    <div class="bg-white/[0.02] border border-white/[0.05] rounded-[16px] md:rounded-[20px] p-4 text-center shadow-xl group transition-all hover:bg-white/[0.04]">
                        <div class="text-[8px] font-black text-white/20 uppercase tracking-[0.2em] mb-1">Hit Rate</div>
                        <div id="userHitRate" class="text-2xl md:text-3xl font-black tracking-tighter text-white">0%</div>
                    </div>
                    <div class="bg-white/[0.02] border border-white/[0.05] rounded-[16px] md:rounded-[20px] p-4 text-center shadow-xl group transition-all hover:bg-white/[0.04]">
                        <div class="text-[8px] font-black text-white/20 uppercase tracking-[0.2em] mb-1">Confirmation</div>
                        <div id="userTotal" class="text-2xl md:text-3xl font-black tracking-tighter text-white">0</div>
                    </div>
                </div>

                <!-- VISUAL CHART -->
                <div class="bg-black/30 border border-white/[0.08] rounded-[16px] md:rounded-[20px] mb-5 md:mb-6 h-[180px] md:h-[200px] overflow-hidden relative shadow-inner" id="userGraphContainer">
                ...
                <div class="bg-white/[0.02] border border-white/[0.05] rounded-[16px] md:rounded-[20px] overflow-hidden shadow-2xl backdrop-blur-md">
                    ...
                    <table class="w-full text-left text-xs">
                        ...
                        <tbody id="userBetsBody" class="divide-y divide-white/[0.03]">
                            <tr><td colspan="4" class="p-6 text-center text-white/30 italic text-xs">Awaiting protocol activity...</td></tr>
                        </tbody>
                    </table>
                </div>
    ...
    window.renderUserAnalytics = function () {
        ...
        if (netEl) {
            ...
            netEl.className = `text-2xl md:text-3xl font-black tracking-tighter ${stats.netUnits > 0 ? 'text-[#30D158]' : (stats.netUnits < 0 ? 'text-[#FF453A]' : 'text-white')}`;
        }
        ...
        if (hrEl) {
            ...
            hrEl.innerText = hr + "%";
            hrEl.className = `text-2xl md:text-3xl font-black tracking-tighter ${total === 0 ? 'text-white' : (hr >= 50 ? 'text-[#30D158]' : 'text-[#FF453A]')}`;
        }
